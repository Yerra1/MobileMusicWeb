...
function navigate(page){$$('.page').forEach(p=>p.classList.toggle('active',p.id===page));window.scrollTo({top:0,behavior:'smooth'})}
function playTrack(i){currentIndex=(i+tracks.length)%tracks.length;const t=tracks[currentIndex];$('#playerTitle').textContent=t.name;$('#playerArtist').textContent=t.artist;$('#heroArt').className=`hero-art ${t.cls}`;$('#progress').value=0;navigate('playing');isPlaying=true;$('#playBtn').textContent='❚❚'}
function togglePlay(){isPlaying=!isPlaying;$('#playBtn').textContent=isPlaying?'❚❚':'▶'}
...
fillHome();fillSearch();fillPlaylists();
